...
{
    /// <summary>
    ///     The Editor for an <see cref="AnimatedTile" />. Mirrors Unity's AnimatedTileEditor but targets our
    ///     <see cref="AnimatedTile" /> (which inherits from <see cref="Tile" />), so the collider type is read
    ///     off the inherited <see cref="Tile.colliderType" /> property instead of the original
    ///     <c>m_TileColliderType</c> field.
    /// </summary>
    ...
    public class AnimatedTileEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(tile.m_AnimatedSprites == null || tile.m_AnimatedSprites.Length == 0))
            {
                ...
                tile.colliderType =
                    (Tile.ColliderType)EditorGUILayout.EnumPopup(Styles.colliderTypeLabel, tile.colliderType);
